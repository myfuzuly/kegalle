...
$brandNames = [
    'Avent','Baby Cheramy','Babyhug','B.Box','Beaba','Bebesup','Bebeconfort',
    'Chicco','Combi','Cybex','Disney Baby','Dr. Brown\'s','Farlin','Fisher-Price',
    'Graco','Huggies','Himalaya Baby','Infantino','Johnson\'s Baby','Joie','LuvLap',
    'MAM','MamyPoko','Mee Mee','Mothercare','Mustela','NUK','Pampers','Panda',
    'Pears','Pigeon','Philips Avent','Playgro','R for Rabbit','Richell',
    'Safety 1st','Sebamed Baby','Softlove','Taf Toys','Tommee Tippee','VTech',
    'Velona Cuddles','Weleda Baby',
];

$cat = Category::where('name', 'like', '%Kids%Baby%')->whereNotNull('parent_id')->first();
if (!$cat) $cat = Category::where('name', 'like', '%Kids%Baby%')->first();
if (!$cat) $cat = Category::where('name', 'like', '%Baby%')->whereNotNull('parent_id')->first();
