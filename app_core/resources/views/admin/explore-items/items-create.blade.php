...
@section('title','Add Item — ' . $explore->title)
@section('page','Explore Kegalle')
@section('heading','Add Item to ' . $explore->title)
@section('subheading','Create a new entry under ' . $explore->title)
@section('actions')<a class="ka-btn ka-btn-light" href="/admin/explore-items/{{ $explore->id }}/items">Back</a>@endsection
@section('content')
<section class="sa-card">
<form class="ka-premium-form" method="post" action="/admin/explore-items/{{ $explore->id }}/items" enctype="multipart/form-data">
@csrf
<div class="ka-form-grid">
    <div class="ka-field ka-span-2">
        <label>Name</label>
        <input name="name" value="{{ old('name') }}" required placeholder="e.g. Pinnawala Elephant Orphanage">
    </div>
    <div class="ka-field ka-span-2">
    ...
    <div class="ka-field">
        ...
        <input name="phone" value="{{ old('phone') }}" placeholder="035-2265284">
    </div>
    <div class="ka-field">
        <label>Email</label>
        <input name="email" type="email" value="{{ old('email') }}" placeholder="info@example.lk">
    </div>
    <div class="ka-field ka-span-2">
        <label>Address</label>
        <input name="address" value="{{ old('address') }}" placeholder="Rambukkana Road, Pinnawala">
    </div>
    <div class="ka-field ka-span-2">
