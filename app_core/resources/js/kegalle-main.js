(function () {
    ...
    // Single unified click handler (J-1 fix: was two separate listeners)
    ...
    // J-6 fix: use onerror instead of 15s timeout to catch failed images
    document.querySelectorAll('.k-listing-img, .k-deal-card-img').forEach(function (el) {
        ...
        if (img.complete) {
            if (img.naturalWidth > 0) { el.style.animation = 'none'; }
    ...
    // J-3 fix: guard against null/undefined before String()
    function sEsc(s) {
        if (s == null) return '';
    ...
    function sRender(data, q) {
        ...
        var rows = data.slice(0, 8).map(function (item) {
            // J-2 fix: validate and escape item.url before inserting into href
            var safeUrl = '';
            try { safeUrl = new URL(item.url, location.origin).href; } catch (e) { safeUrl = '/listings'; }
            return '<a class="k-search-item" href="' + sEsc(safeUrl) + '">'
                + '<span class="k-search-item-icon">🔍</span>'
                + '<span class="k-search-item-title">' + sHl(item.title, q) + '</span>'
                + (item.category ? '<span class="k-search-item-cat">' + sEsc(item.category) + '</span>' : '')
                + '</a>';
        }).join('');
        rows += '<a class="k-search-all" href="/listings?q=' + encodeURIComponent(q) + '">See all results for "<strong>' + sEsc(q) + '</strong>" →</a>';
    ...
    if (sInput && sDrop) {
        ...
        // J-5 fix: only call sClose when dropdown is actually open
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // J-4 fix: removed dead form.matches('form') check — e.target is always a form element
